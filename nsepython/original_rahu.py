...
def oi_chain_builder(symbol, expiry="latest", oi_mode="full"):
    ...
    for m in range(len(payload["records"]["data"])):
        if payload["records"]["data"][m]["expiryDate"] == expiry:
            if 1 > 0:
                try:
                    oi_row["CALLS_OI"] = payload["records"]["data"][m]["CE"][
                        "openInterest"
                    ]
                    oi_row["CALLS_Chng in OI"] = payload["records"]["data"][m]["CE"][
                    ...
                    oi_row["CALLS_LTP"] = payload["records"]["data"][m]["CE"][
                        "lastPrice"
                    ]
                    oi_row["CALLS_Net Chng"] = payload["records"]["data"][m]["CE"][
                        "change"
                    ]
                    if oi_mode == "full":
                        oi_row["CALLS_Bid Qty"] = payload["records"]["data"][m]["CE"][
                            "bidQty"
                        ]
                        oi_row["CALLS_Bid Price"] = payload["records"]["data"][m]["CE"][
                            "bidprice"
                        ]
                        oi_row["CALLS_Ask Price"] = payload["records"]["data"][m]["CE"][
                            "askPrice"
                        ]
                        oi_row["CALLS_Ask Qty"] = payload["records"]["data"][m]["CE"][
                            "askQty"
                        ]
                except KeyError:
                    (
                        oi_row["CALLS_OI"],
                        oi_row["CALLS_Chng in OI"],
                        oi_row["CALLS_Volume"],
                        oi_row["CALLS_IV"],
                        oi_row["CALLS_LTP"],
                        oi_row["CALLS_Net Chng"],
                    ) = (0, 0, 0, 0, 0, 0)
                    if oi_mode == "full":
                        (
                            oi_row["CALLS_Bid Qty"],
                            oi_row["CALLS_Bid Price"],
                            oi_row["CALLS_Ask Price"],
                            oi_row["CALLS_Ask Qty"],
                        ) = (0, 0, 0, 0)
                    pass
                ...
                try:
                    oi_row["PUTS_OI"] = payload["records"]["data"][m]["PE"][
                        "openInterest"
                    ]
                    oi_row["PUTS_Chng in OI"] = payload["records"]["data"][m]["PE"][
                        "changeinOpenInterest"
                    ]
                    ...
                    oi_row["PUTS_LTP"] = payload["records"]["data"][m]["PE"][
                        "lastPrice"
                    ]
                    oi_row["PUTS_Net Chng"] = payload["records"]["data"][m]["PE"][
                        "change"
                    ]
                    if oi_mode == "full":
                        oi_row["PUTS_Bid Qty"] = payload["records"]["data"][m]["PE"][
                            "bidQty"
                        ]
                        oi_row["PUTS_Bid Price"] = payload["records"]["data"][m]["PE"][
                            "bidprice"
                        ]
                        oi_row["PUTS_Ask Price"] = payload["records"]["data"][m]["PE"][
                            "askPrice"
                        ]
                        oi_row["PUTS_Ask Qty"] = payload["records"]["data"][m]["PE"][
                            "askQty"
                        ]
                except KeyError:
                    (
                        oi_row["PUTS_OI"],
                        oi_row["PUTS_Chng in OI"],
                        oi_row["PUTS_Volume"],
                        oi_row["PUTS_IV"],
                        oi_row["PUTS_LTP"],
                        oi_row["PUTS_Net Chng"],
                    ) = (0, 0, 0, 0, 0, 0)
                    if oi_mode == "full":
                        (
                            oi_row["PUTS_Bid Qty"],
                            oi_row["PUTS_Bid Price"],
                            oi_row["PUTS_Ask Price"],
                            oi_row["PUTS_Ask Qty"],
                        ) = (0, 0, 0, 0)
            else:
                logging.info(m)
...
def nse_expirydetails(payload, i=0):
    ...
    currentExpiry = datetime.datetime.strptime(
        currentExpiry, "%d-%b-%Y",
    ).date()  # converting json datetime to alice datetime
...
def nse_results(index="equities", period="Quarterly"):
    if (index == "equities") or (index == "debt") or (index == "sme"):
        if (
            (period == "Quarterly")
            or (period == "Annual")
            or (period == "Half-Yearly")
            or (period == "Others")
        ):
            payload = nsefetch(
                "https://www.nseindia.com/api/corporates-financial-results?index="
                + index
                + "&period="
                + period,
            )
...
def nse_fiidii(mode="pandas"):
    try:
        if mode == "pandas":
            return pd.DataFrame(
                nsefetch("https://www.nseindia.com/api/fiidiiTradeReact"),
            )
        else:
...
def equity_history(symbol, series, start_date, end_date):
    ...
    for i in range(0, int(diff.days / 40)):
        ...
        total = pd.concat(
            [total, equity_history_virgin(symbol, series, start_date, temp_date)],
        )
    ...
    total = pd.concat(
        [total, equity_history_virgin(symbol, series, start_date, end_date)],
    )
...
def derivative_history_virgin(
    ...
    if ((instrumentType == "OPTIDX") or (instrumentType == "OPTSTK")) and (
        expiry_date != ""
    ):
...
def expiry_history(symbol, start_date="", end_date=""):
    if end_date == "":
        end_date = end_date
    nsefetch_url = (
        "https://www.nseindia.com/api/historical/fo/derivatives/meta?&from="
        + start_date
        + "&to="
        + end_date
        + "&symbol="
...
def nse_preopen_movers(key="FO", filter=1.5):
    ...
    return (
        preOpen_gainer[preOpen_gainer["pChange"] > 1.5],
        preOpen_gainer[preOpen_gainer["pChange"] < -1.5],
    )
...
def nse_most_active(type="securities", sort="value"):
    payload = nsefetch(
        "https://www.nseindia.com/api/live-analysis-most-active-"
        + type
        + "?index="
        + sort
        + "",
