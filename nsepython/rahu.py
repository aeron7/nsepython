...
if (mode == 'local'):
    def nsefetch(url, cookies):
        response = requests.get(
            url, timeout=30, headers=headers, cookies=cookies)
...
def running_status():
    start_now = datetime.datetime.now().replace(
        hour=9, minute=15, second=0, microsecond=0)
    end_now = datetime.datetime.now().replace(
...
def nse_quote(symbol, section=""):
    ...
    if (section != ""):
        payload = nsefetch(
            'https://www.nseindia.com/api/quote-equity?symbol='+symbol+'&section='+section)
        return payload


def nse_expirydetails(payload, i=0):
    currentExpiry = payload['records']['expiryDates'][i]
    # converting json datetime to alice datetime
    currentExpiry = datetime.datetime.strptime(
        currentExpiry, '%d-%b-%Y').date()
...
def black_scholes_dexter(S0, X, t, σ="", r=10, q=0.0, td=365):
    ...
    S0, X, σ, r, q, t = float(S0), float(X), float(
        σ/100), float(r/100), float(q/100), float(t/td)
    ...
    d2 = d1-σ*math.sqrt(t)
    Nd2 = norm.cdf(d2)
    call_theta = (-((S0*σ*math.exp(-q*t))/(2*math.sqrt(t))*(1/(math.sqrt(2*math.pi))) *
                  math.exp(-(d1*d1)/2))-(r*X*math.exp(-r*t)*norm.cdf(d2))+(q*math.exp(-q*t)*S0*norm.cdf(d1)))/td
    put_theta = (-((S0*σ*math.exp(-q*t))/(2*math.sqrt(t))*(1/(math.sqrt(2*math.pi))) *
                 math.exp(-(d1*d1)/2))+(r*X*math.exp(-r*t)*norm.cdf(-d2))-(q*math.exp(-q*t)*S0*norm.cdf(-d1)))/td
    call_premium = math.exp(-q*t)*S0*norm.cdf(d1)-X * \
        math.exp(-r*t)*norm.cdf(d1-σ*math.sqrt(t))
    put_premium = X*math.exp(-r*t)*norm.cdf(-d2) - \
        math.exp(-q*t)*S0*norm.cdf(-d1)
    call_delta = math.exp(-q*t)*norm.cdf(d1)
    put_delta = math.exp(-q*t)*(norm.cdf(d1)-1)
    gamma = (math.exp(-r*t)/(S0*σ*math.sqrt(t))) * \
...
def equity_history_virgin(symbol, series, start_date, end_date):
    url = "https://www.nseindia.com/api/historical/cm/equity?symbol="+symbol + \
        "&series=[%22"+series+"%22]&from=" + \
        str(start_date)+"&to="+str(end_date)+""
    payload = nsefetch(url, fetch_cookies())
    # Using only a few columns here
    columns = ['CH_TIMESTAMP', 'CH_OPENING_PRICE', 'CH_TRADE_HIGH_PRICE',
               'CH_CLOSING_PRICE', 'CH_TRADE_LOW_PRICE']
    return pd.DataFrame.from_records(payload["data"])[columns]
...
def equity_history(symbol, series, start_date, end_date):
    ...
    for i in range(0, int(diff.days/40)):
        ...
        temp_date = (start_date+datetime.timedelta(days=(40))
                     ).strftime("%d-%m-%Y")
        ...
        total = pd.concat([total, equity_history_virgin(
            symbol, series, start_date, temp_date)])
    ...
    total = pd.concat([total, equity_history_virgin(
        symbol, series, start_date, end_date)])
...
def derivative_history(symbol, start_date, end_date, instrumentType, expiry_date, strikePrice="", optionType=""):
    ...
    for i in range(0, int(diff.days/40)):
        ...
        total = pd.concat([total, derivative_history_virgin(
            symbol, start_date, temp_date, instrumentType, expiry_date, strikePrice, optionType)])
    ...
    total = pd.concat([total, derivative_history_virgin(
        symbol, start_date, end_date, instrumentType, expiry_date, strikePrice, optionType)])
...
def expiry_history(symbol, start_date="", end_date=""):
    if (end_date == ""):
        end_date = end_date
    nsefetch_url = "https://www.nseindia.com/api/historical/fo/derivatives/meta?&from=" + \
        start_date+"&to="+end_date+"&symbol="+symbol+""
    payload = nsefetch(nsefetch_url)
...
def get_beta_df_maker(symbol, days):
    if ("NIFTY" in symbol):
        ...
        # thispointer.com/drop-first-row-of-pandas-dataframe-3-ways/
        df = df.iloc[1:, :]
        return df
...
def nse_preopen(key="NIFTY", type="pandas"):
    payload = nsefetch(
        "https://www.nseindia.com/api/market-data-pre-open?key="+key+"")
    if (type == "pandas"):
        payload = pd.DataFrame(payload['data'])
...
def nse_most_active(type="securities", sort="value"):
    payload = nsefetch(
        "https://www.nseindia.com/api/live-analysis-most-active-"+type+"?index="+sort+"")
    payload = pd.DataFrame(payload["data"])
    return payload
